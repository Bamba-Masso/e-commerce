  ...
  if(!empty($_POST['name']) && !empty($_POST['description']) && !empty($_POST['categorie']) && !empty($_POST['price']) &&!empty($_POST['picture'])){
    $name=$_POST['name'];
     ...
     $picture=$_POST['picture'];
      ...
      if($recup){
       $insert='Le produit a été bien ajouter';
      }  }else{
     $error=" Desolé le produit n'a été ajouter ";
    }
    ...
    <?php 
     ...
     <!-- <div class="mb-4">
      ...
      <div class="flex items-center">
       ...
       <input class="cursor-pointer rounded bg-purple-600 py-2 px-8 text-center text-lg font-bold  text-white" type="submit" value="Ajouter" >
      </div>
    </form>
  </div>
</div>  
